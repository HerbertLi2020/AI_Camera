...
import os, sys, time, cv2, threading, pynvml
...
try: #检测显卡是否是Nvidia，并查看型号是否支持
    ...
    if ('GTX' in GPU_name or 'RTX' in GPU_name) : if_use_gpu = True; GPU_Ready = True
...
#读取配置文件icanx.ini
try:
    config_file = open('icanx.ini', 'r', encoding='utf-8')
    config_dict = {}
...
class Winshot(QWidget):
    def __init__(self):
        ...
        self.setWindowTitle("iCANX虚拟会议系统(视频背景替换工具)")
    ...
    def show_error(self,str):
        r_button = QMessageBox.question(self, "iCANX虚拟会议系统：视频背景替换工具",
                                        '\n\n'+str+'\n\n', QMessageBox.Ok)
    ...
    def helpWin(self):
        ...
        "      本软件著作权归属：iCanX        网址：talks.ican-x.com\n\n"
        QMessageBox.question(self, "iCANX虚拟会议系统：视频背景替换工具", str, QMessageBox.Ok)
    def quitWin(self):
        r_button = QMessageBox.question(self, "iCANX虚拟会议系统：视频背景替换工具",
                                        "\n\n    退出将终止替换进程\n\n    确认退出吗？\n\n", QMessageBox.Yes | QMessageBox.No)
    ...
    def box_choose(self):
        ...
        else:Box1_flag = False;  self.my_label1.setPixmap(QPixmap("ican_logo.png"))
        if self.checkBox2.isChecked(): Box2_flag = True
        else:Box2_flag = False;  self.my_label2.setPixmap(QPixmap("ican_logo.png"))
